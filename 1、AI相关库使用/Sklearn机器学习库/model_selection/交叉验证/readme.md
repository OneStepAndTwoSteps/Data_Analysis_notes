...
*   `留一法`：交叉验证法的一个特例，假定数据集 D 中包含 m 个样本，若令 k=m(样本数)，则为留一法，留一法不随随机样本划分影响，这样对于m个样本，每次选择m-1个样本来训练数据，留一个样本来验证模型预测的好坏。此方法主要用于样本量非常少的情况，比如对于普通适中问题，m小于50时，我一般采用留一交叉验证。
