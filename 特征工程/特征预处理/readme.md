...
这是最常见的特征预处理方式，基本所有的线性模型在拟合的时候都会做 z-score标准化。具体的方法是求出样本特征x的均值mean和标准差std，然后用（x-mean)/std来代替原特征。这样特征就变成了均值为0，方差为1了。 __联想到标准正态分布（standard normal distribution）的特征也是均值为0、方差为1，觉得这就是被称为标准化的原因。__
