...
* `CNN` 在进行反向传播时，后一层 $\delta^t$ 计算 $\delta^{(t-1)}$ 时，卷积核需要旋转180度，就可以从转置卷积的角度进行分析，`前向传播`：高维 --> 低维，`反向传播`：低维 --> 高维，通过转置卷积得到。
  
* `抽丝剥茧，带你理解转置卷积（反卷积）：`https://blog.csdn.net/tsyccnh/article/details/87357447?spm=1001.2014.3001.5506


## `卷积 之后经过 激活函数 得到的 激活值 和 卷积核-图像 之间的联系：`

* 卷积的操作是通过滑动窗口进行的，每次滑动窗口都和卷积核做卷积运算得到值，这个是对的。但是之后不是找到其中的最大的值位置，而是把所有的得到的值都保留下来，形成一个通道（特征图）。这个通道（特征图）就表示了输入图像在该卷积核下的响应程度，也就是输入图像有多大程度上包含了该卷积核所对应的视觉模式或概念。然后可以通过查看通道（特征图）中的值最大的位置，找到对应的滑动窗口位置，那么这个位置所对应的滑动窗口位置，就是这个卷积核最敏感的。然后可以通过查看滑动窗口中的图像大致是什么，来判断卷积核对什么图像敏感。
