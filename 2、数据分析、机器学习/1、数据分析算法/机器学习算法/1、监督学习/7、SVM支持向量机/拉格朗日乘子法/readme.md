...
*  当我们有如下 `约束函数` 时：
    ...
    * 为什么等价呢，我们来看一下，首先我们将原来的约束问题转成了图片2中的无约束问题，这个时候图片2中的x其实就是无约束的了，可以去全空间，那么我们现在开始分情况讨论，分别假设 `x` 在 `图片1原问题的可行域` 和 `不在图片1原问题的可行域` 时，有如下情况：
